...
-- CONFIG
JTAC_maxDistance = 4500 -- How far a JTAC can "see" in meters (with LOS)

JTAC_smokeOn = false -- enables marking of target with smoke, can be overriden by the JTACAutoLase in editor

JTAC_smokeColour = 4 -- Green = 0 , Red = 1, White = 2, Orange = 3, Blue = 4
  ...
  --      env.info("TEMP UNIT INFO: " .. tempUnitInfo.name .. " " .. tempUnitInfo.unitType)
    ...
    if enemyUnit == nil then
        ...
        if enemyUnit ~= nil then
            ...
            notify(jtacGroupName .. " lasing new target " .. enemyUnit:getTypeName() .. '. LSR: ' .. laserCode ..getPositionString(enemyUnit) , 10)
...
function notify(message, displayFor, groupId)
	...
	if groupId then
		...
		trigger.action.outSoundForGroup(groupId, "radiobeep.ogg")
	else
    ...
    for jtacGroupName, jtacUnitName in pairs(GLOBAL_JTAC_UNITS) do
        ...
        if jtacUnit ~= nil and jtacUnit:getLife() > 0 and jtacUnit:isActive() == true then
            ...
            if enemyUnit ~= nil and enemyUnit:getLife() > 0 and enemyUnit:isActive() == true then
                message = message .. "" .. jtacGroupName .. " | TGT:" .. enemyUnit:getTypeName().. " | LSR:".. laserCode .. getPositionString(enemyUnit) .. "\n"
    ...
    if blueGroups ~= nil then
        for x, tmpGroup in pairs(blueGroups) do
            ...
            if GLOBAL_JTAC_RADIO_ADDED[index] == nil then
                ...
                missionCommands.addCommandForGroup(Group.getID(tmpGroup), "JTAC Status", nil, getJTACStatus, Group.getID(tmpGroup))
